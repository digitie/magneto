...
class PlotPopup(wx.Menu):
	def __init__(self, canvas, axes):
		wx.Menu.__init__(self)

		self.canvas = canvas
		self.axes = axes
		if self.canvas.limits.have_history():
			item = wx.MenuItem(self, wx.NewId(), "&Zoom out")
			self.AppendItem(item)
			self.Bind(wx.EVT_MENU, self.OnZoomOut, item)
	...
	def OnZoomOut(self, event):
		self.canvas.zoom_out(self.axes)

	def OnShowGrid(self, event):
		self.canvas.grid = not self.canvas.grid
		if self.canvas.grid:
			self.axes.grid(True, color='gray')
		else:
			self.axes.grid(False)
		self.canvas.forceDraw()
...
class PlotPanel(FigureCanvasWxAgg):
	...
	*figpx* is a tuple with the horizontal and vertical size of the figure 
	...
	def __init__(self, parent, id, 
					cursor=True, crosshairs=False, rubberband=True, point=True, selection=True, zoom=True, grid = True, 
	...
	def on_button_press_event(self, event):
		...
		if event.button==1:
			...
			menu = PlotPopup(canvas = self, axes = axes)
			size = self.GetSize()
			self.PopupMenu(menu, wx.Point(event.x, size.GetHeight() - event.y))
			menu.Destroy()
			'''
			if axes is not None:
	...
	def on_pick_event(self, event):
		...
		origline = self.linedict[legline]
	...
	def draw_legend(self, merge = False):
		if merge:
			lines = []
			...
			leg = ax.legend(lines, labs, loc=0, framealpha=0, prop={'size':8})
...
class PlotFrame(wx.Frame):
	...
	*lock* is a :class:`Lock` object that prevents other threads from accessing 
	...
	def create_menus(self):
		...
		id = wx.NewId()
		menu.Append(id, '&Zoom to Fit...', 'Zoom to fit current display')
		wx.EVT_MENU(self, id, self.OnZoomToFit)

		mainMenu.Append(menu, '&Graph')
